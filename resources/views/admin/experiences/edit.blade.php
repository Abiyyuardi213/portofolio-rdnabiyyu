...
@php
    ...
    if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
        $description = collect($decoded)->map(fn ($item) => '<p>' . e($item) . '</p>')->implode('');
    }
@endphp
    ...
    <form action="{{ route('experiences.update', $experience->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-with-preview">
                    ...
                    <div class="form-group">
                        ...
                        <label class="upload-control">
                            <span class="upload-meta">
                                ...
                                <span class="upload-filename" data-logo-filename>{{ $experience->logo ? basename($experience->logo) : 'No file selected' }}</span>
                            </span>
                        ...
                        @if($experience->logo)
                            <p class="card-subtitle" style="margin-top: 0.5rem;">Current: {{ $experience->logo }}</p>
                        @endif
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="text-editor" data-text-editor>
                        ...
                        <div class="text-editor-area" contenteditable="true" data-editor-area data-placeholder="Tulis deskripsi pengalaman...">{!! $description !!}</div>
            ...
            <aside class="crop-panel">
                <h4>Logo Crop</h4>
                <div class="crop-frame {{ $experience->logo ? 'has-image' : '' }}" data-crop-frame>
                    <span>Pilih logo untuk preview crop</span>
                    <img src="{{ $experience->logo ? asset($experience->logo) : '' }}" alt="Logo preview" data-logo-preview>
                </div>
...
<script>
    ...
    function writeCroppedLogo() {
        if (!logoInput.files?.length || !preview?.complete || !preview.naturalWidth) return;
